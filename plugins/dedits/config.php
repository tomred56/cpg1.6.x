...
function plg_dedits_config_save ($sC)
{
	global $lang_plugin_dedits, $lang_common, $lang_pluginmgr_php;

	// Get configuration values and save them
	$optval = $sC->post->getEscaped('cfgopt');
	cpg_config_set('plg_dedits_optval', $optval, true);

	// Display a success message
	starttable('100%', $lang_common['information']);
	$msg = sprintf($lang_pluginmgr_php['saved_config'], $lang_plugin_dedits['config_name']);
	echo <<<EOT
	<tr>
...
function plg_dedits_config_form ($sC)
{
	global $CONFIG, $lang_common, $lang_plugin_dedits, $lang_gallery_admin_menu;
	
	$submit_icon = cpg_fetch_icon('ok', 1);

	// Provide a configuration table header
	starttable('100%', $lang_plugin_dedits['config_name'].' - '.$lang_gallery_admin_menu['admin_lnk'], 2);

	// Display rows for config items and a save button
	echo <<<EOT
	<tr>
		<td class="tableb" width="40%">Configuration phrase:</td>
		<td class="tableb"><input type="text" class="text" name="cfgopt" value="{$CONFIG['plg_dedits_optval']}" /></td>
	</tr>
	<tr>
...
if ($superCage->post->keyExists('save'))
	plg_dedits_config_save($superCage);

plg_dedits_config_form($superCage);
