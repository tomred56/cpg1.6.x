...
function theme_display_thumbnails(&$thumb_list, $nbThumb, $album_name, $aid, $cat, $page, $total_pages, $sort_options, $display_tabs, $mode = 'thumb', $date='')
{
    ...
    foreach($thumb_list as $thumb) {
		...
		$pid = $thumb['pid'];
        $result = cpg_db_query("SELECT user4 FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE pid = $pid $FORBIDDEN_SET LIMIT 1");
		$row = $result->fetchAssoc();
		$item = $row['user4'];
		if ($item <> '') {
			$thumb['caption'] .= "($item)&emsp;";
		}
...
<!-- BEGIN img_desc -->
                            <table class="img_caption_table"  height="100%" cellspacing="2" cellpadding="0">
<!-- BEGIN title -->
								<thead align="center">
									<th >
										<h1 class="pic_title">{TITLE} 
										</h1> <br />
									</th>
								</thead>
<!-- END title -->
...
<!-- BEGIN caption -->
								<tbody align="left">
									<td >
										<h2 class="pic_detail">{CAPTION}
										</h2><br />
										<h2 class="pic_detail">{OWNER}
										</h2><br />
										<h2 class="pic_detail">{INSURANCE}
										</h2><br />
										<h2 class="pic_detail">{LOCATION}
										</h2><br />
										<h2 class="pic_detail"> {THUMBS} 
										</h2>
									</td>
                                </tbody>
<!-- END caption -->
...
function theme_html_picture()
{
    ...
    $CURRENT_PIC_DATA['menu'] = html_picture_menu('menu'); //((USER_ADMIN_MODE && $CURRENT_ALBUM_DATA['category'] == FIRST_USER_CAT + USER_ID) || ($CONFIG['users_can_edit_pics'] && $CURRENT_PIC_DATA['owner_id'] == USER_ID && USER_ID != 0) || GALLERY_ADMIN_MODE) ? html_picture_menu($pid) : '';
	...
	if ($CURRENT_PIC_DATA['user4'] <> '') {
		$pid = $CURRENT_PIC_DATA['pid'];
		if (!is_numeric($CURRENT_ALBUM_DATA['aid'])) {
			$aid = "";
		}
		elseif ($CURRENT_ALBUM_DATA['aid'] <> $CURRENT_PIC_DATA['aid']) {
			$aid = "album=" . $CURRENT_PIC_DATA['aid'] ."&amp;";
		}
		else {
			$aid = "album=" . $CURRENT_ALBUM_DATA['aid'] ."&amp;";
		}
	    $id = $CURRENT_PIC_DATA['user4'];
		$title = "($id)&emsp;$title";
       $result = cpg_db_query("SELECT pid,aid,title,filepath,filename,pwidth,pheight FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE user4 = '$id' and pid <> $pid $FORBIDDEN_SET LIMIT 10");

       if ($result->numRows() == 0) {
		   $altpics = 'No other pictures found';
	   }
	   else{
		   while ( ($row = $result->fetchAssoc()) ) {

			 $url = "displayimage.php?" . "cat=0&amp;pid=" . $row['pid'] . "#top_display_media";
//			 $url = "displayimage.php?" . $aid . "cat=0&amp;pid=" . $row['pid'] . "#top_display_media";
//			 $url = "displayimage.php?album=" . $row['aid'] . "&amp;cat=0&amp;pid=" . $row['pid'] . "#top_display_media";
//			 $url = "displayimage.php?pid=" . $row['pid'];
			 $url1 = $CONFIG['ecards_more_pic_target'] . $url;
			 $img = $CONFIG['ecards_more_pic_target'] . $CONFIG['fullpath'] . $row['filepath'] . $CONFIG['thumb_pfx'] . $row['filename'];
			 $width = $row['pwidth'];
			 $height = $row['pheight'];
			 $id =uniqid(rand());
			 $altpic= "<a href=\"$url\" class=\"thumbLink\" style=\"width:128px; float: left\"><img src=\"$img\" class=\"strip_image\" border=\"0\" alt=\"Click to view this image\" /></a>";         
//			 $altpic= "<a href=\"javascript:;\"  onclick=\"MM_openBrWindow('$url&amp;fullsize=1','$id','scrollbars=yes,toolbar=no,status=no,resizable=yes,width=$width,height=$height')\"><img src=\"$img\" class=\"image\" border=\"0\" alt=\"Click to view full size image\" /></a>";         
//			 $altpic= "<a href=\"javascript:;\" onclick=\"MM_openBrWindow('$url1&amp;fullsize=0') ondblclick=\"MM_openBrWindow('$url&amp;fullsize=1','$id','scrollbars=yes,toolbar=no,status=no,resizable=yes,width=$width,height=$height')\"><img src=\"$img\" class=\"image\" border=\"0\" alt=\"Click to view full size image\" /></a>";         
//			<a href="displayimage.php?album=search&amp;cat=0&amp;pid=339#top_display_media" class="thumbLink" style="width:128px; float: left"><img src="albums/Jewelery/thumb_20190602_145936.jpg" class="strip_image" border="0" alt="20190602_145936.jpg" title="Filename=20190602_145936.jpg
//Filesize=136KiB
//Dimensions=1067x702
//Date added=Jun 03, 2019"></a>
		...
		$altpics = 'Reference field must be entered to locate other pictures.';
    }
