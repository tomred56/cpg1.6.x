...
function theme_html_picture()
{
    ...
    $thumbs='';
/*
    while (stripos($cap,'displayimage.php') > 0) {
         $startu1 = stripos($cap,'[url=');
         $startu2 = stripos($cap,'displayimage.php',$startu1);
         $endu=stripos($cap,']',$startu2);
         $starti1 = stripos($cap,'[img]',$endu);
         $starti2 = stripos($cap,'albums/',$starti1);
         $endi=stripos($cap,'[/img]',$starti2);
         $end=stripos($cap,'[/url]',$endi)+6;
         $url=substr($cap,$startu2,$endu-$startu2);
         $img=substr($cap,$starti2,$endi-$starti2);
         $img=str_ireplace('normal','thumb',$img);
         $cap=substr_replace($cap,'',$startu1,$end-$startu1);
         $id =uniqid(rand());
         $thumb= "<a href=\"javascript:;\" onclick=\"MM_openBrWindow('$url&amp;fullsize=1','$id','scrollbars=yes,toolbar=no,status=no,resizable=yes,width=2000,height=2000')\"><img src=\"$img\" class=\"image\" border=\"0\" alt=\"Click to view full size image\" /></a>";         
         $thumbs .= $thumb;
     }
*/
...
/* 
 if (preg_match("/^\([a-zA-Z]\d{3}\)/",$CURRENT_PIC_DATA['title'])) {
		$pid = $CURRENT_PIC_DATA['pid'];
	   $picid = substr($CURRENT_PIC_DATA['title'], 0, 6) . '%';
       $result = cpg_db_query("SELECT pid,title,filepath,filename,pwidth,pheight FROM {$CONFIG['TABLE_PICTURES']} AS p WHERE title like '$picid' and pid <> $pid $FORBIDDEN_SET LIMIT 5");
       ...
       if ($result->numRows() == 0) {
		...
		$thumbs = 'Title must start with a reference "(A000)" to locate other pictures.';
    }
*/
